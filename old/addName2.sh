...
while (( "$#" )); do
    ...
    if [ -z "$file" ]; then
        ...
        exiftool -codedcharacterset=utf8 -exif:xpkeywords+="$name" -iptc:keywords+="$name" -ext jpg .
        ...
        # Add IPTC keyword. Method to detect corruption.
        SIZEFILE_ORIG=$(stat -c '%s' "$file")
        exiftool -codedcharacterset=utf8 -exif:xpkeywords+="$name" -iptc:keywords+="$name" -ext jpg "$file"
        SIZEFILE_NEW=$(stat -c '%s' "$file")
